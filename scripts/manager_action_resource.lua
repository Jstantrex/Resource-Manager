		...
		if (nActive ~= 0) then
			...
			for kEffectComp,sEffectComp in ipairs(aEffectComps) do
				...
				if rEffectComp.type == "IF" then
					if not EffectManager35E.checkConditional(rSource, nodeEffect, rEffectComp.remainder) then
						break;
					end
				elseif rEffectComp.type == "IFT" then
					...
					if not EffectManager35E.checkConditional(rTarget, nodeEffect, rEffectComp.remainder, rSource) then
						break;
					end
					bTargeted = true;
...
function handleResourceHeal(msgOOB)
	local rSource = ActorManager.resolveActor(msgOOB.sSourceNode);
	local rTarget = ActorManager.resolveActor(msgOOB.sTargetNode);
	local rEffectComp = EffectManager35E.parseEffectComp(msgOOB.sEffectComp);
